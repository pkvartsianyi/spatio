...
async fn test_remote_client_ops() -> anyhow::Result<()> {
    ...
    tokio::spawn(async move {
        // Use spatio_server library
        let _ = spatio_server::run_server(
            bound_addr,
            db_for_server,
    ...
    assert_eq!(result.0.x(), -74.0060);
    assert_eq!(result.1["pop"], 8000000);

    // 5. Verify stats
    let stats = client.stats().await?;
    assert_eq!(stats.hot_state_objects, 1);

    // 6. Query radius
    let nyc_3d = Point3d::new(-74.0060, 40.7128, 0.0);
    let nearby = client
        .query_radius("cities", &nyc_3d, 100_000.0, 10)
        .await?;
    assert_eq!(nearby.len(), 1);
    assert_eq!(nearby[0].0, "nyc");
