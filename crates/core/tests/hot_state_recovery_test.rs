...
mod tests {
    ...
    fn test_db_restart_preserves_current_locations() {
        ...
        // Create DB and insert locations
        {
            ...
            // Verify before close
            let results = db
                .query_radius("vehicles", &Point3d::new(-74.0, 40.0, 0.0), 50000.0, 10)
                .unwrap();
        ...
        // Reopen DB
        {
            ...
            let aircraft = db
                .query_radius("aircraft", &Point3d::new(-75.0, 41.0, 5000.0), 10000.0, 10)
                .unwrap();
            assert_eq!(aircraft.len(), 1, "Should recover 1 aircraft");

            // Verify specific object
            let (truck, _) = vehicles
                .iter()
                .find(|(v, _)| v.object_id == "truck_001")
                .unwrap();
